<template>
    <div>
      ...
      <Amoozeshi v-if="all_contents != null" :contents="all_contents" />
    </div>
</template>
  
  <script>
  import { mapGetters } from "vuex";
  import Amoozeshi from "@/components/Amoozeshi/Amoozesh.vue";
  ...
  export default {
    ...
    methods: {
      async getContent() {
        const loadData = await this.$axios.get("/api/contents/get-special-content");
        // console.log(loadData)
        if (loadData.data != null) {
          this.loading = false;
          this.all_contents = loadData.data.specialList;
        } else {
