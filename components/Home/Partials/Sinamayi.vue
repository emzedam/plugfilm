  ...
  <section class="movies-slider my-20 px-6 overflow-hidden relative">
    ...
    >
      <div class="flex items-center justify-between">
        <h2 class="lg:text-2xl text-white"> فیلم سینمایی </h2>
        <span class="lg:text-xl text-sm mr-4 text-gray-200 flex items-center"
          ><nuxt-link to="/content/movies" class="lg:block hidden" >مشاهده بیشتر</nuxt-link
      ...
      <div class="slider-btn pr-0 relative overflow-hidden items-center flex">
        ...
        >
          <div class="prev8 w-10 cursor-pointer">
            <i class="fa-solid fa-chevron-right"></i>
          </div>
          <div class="next8 w-10 cursor-pointer">
              ...
              >
                ...
                >
                  <img
                    ...
                    onerror="javascript:this.src='/logo.png'"
                    :alt="movie.FaName"
                    :title="movie.FaName"
                    v-lazy-load
                  ...
                  <figcaption>
                    ...
                    >
                      <ul class="inline-flex gap-x-4 list-disc" v-if="movie.categories.length != 0">
                        <li v-for="(cat , index) in movie.categories" :key="index" class="hover:text-white">{{ cat.title }}</li>
                      </ul>
                    </div>
                  </figcaption>
                </figure>
...
export default {
  ...
  async mounted() {
    await this.$nextTick();
    new Swiper(this.$refs.swiper_updates, {
      slidesPerView: 6,
      loop: false,
      dir: "ltr",
      spaceBetween: 15,
      navigation: { nextEl: ".next8", prevEl: ".prev8" },
